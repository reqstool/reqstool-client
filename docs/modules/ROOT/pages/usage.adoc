...
** `config` -- `configuration`: fulfilled by application config (YAML properties, env vars); no annotation required
...
The `In Code` panel shows total, verified, annotated-not-verified, and missing-annotation counts.
The `N/A`, `Configuration`, `Platform`, and `Framework` panels each show total, verified, and not-verified counts.
